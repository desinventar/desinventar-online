...
<script language="php">
	...
	$r->copyCauses('eng');
	$i = new DIImport($us);
	//$a = $i->importFromCSV('/tmp/LKA_event.csv', DI_EVENT, true, 0);
	//$a = $i->importFromCSV('/tmp/LKA_cause.csv', DI_CAUSE, true, 0);
	//$a = $i->importFromCSV('/tmp/LKA_geolevel.csv', DI_GEOLEVEL, true, 0);
	//$a = $i->importFromCSV('/tmp/LKA_geography.csv', DI_GEOGRAPHY, true, 0);
	//$a = $i->importFromCSV('/tmp/LKA_disaster.csv', DI_DISASTER, true, 0);

	//createEEFields($us, '/tmp/LKA_eefield.csv','/tmp/LKA_eedata.csv');
	//importEEData($us,'/tmp/LKA_eedata.csv');
