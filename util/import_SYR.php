...
<script language="php">
	...
	//$r->copyCauses('eng');
	$i = new DIImport($us);
	//$a = $i->importFromCSV('/tmp/SYR_event.csv', DI_EVENT, true, 0);
	//$a = $i->importFromCSV('/tmp/SYR_cause.csv', DI_CAUSE, true, 0);
	//$a = $i->importFromCSV('/tmp/SYR_geolevel.csv', DI_GEOLEVEL, true, 0);
	//$a = $i->importFromCSV('/tmp/SYR_geography.csv', DI_GEOGRAPHY, true, 0);
	//$a = $i->importFromCSV('/tmp/SYR_disaster.csv', DI_DISASTER, true, 0);

	//createEEFields($us, '/tmp/SYR_eefield.csv','/tmp/SYR_eedata.csv');
	//importEEData($us,'/tmp/SYR_eedata.csv');
