...
<script language="php">
	...
	//$r->copyCauses('spa');
	$i = new DIImport($us);
	//$a = $i->importFromCSV('/tmp/NIC_event.csv', DI_EVENT, true, 0);
	//$a = $i->importFromCSV('/tmp/NIC_cause.csv', DI_CAUSE, true, 0);
	//$a = $i->importFromCSV('/tmp/NIC_geolevel.csv', DI_GEOLEVEL, true, 0);
	//$a = $i->importFromCSV('/tmp/NIC_geography.csv', DI_GEOGRAPHY, true, 0);
	//$a = $i->importFromCSV('/tmp/NIC_disaster.csv', DI_DISASTER, true, 0);

	//createEEFields($us, '/tmp/NIC_eefield.csv','/tmp/NIC_eedata.csv');
	//importEEData($us,'/tmp/NIC_eedata.csv');
