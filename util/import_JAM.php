...
<script language="php">
	...
	//$r->copyEvents('eng');
	//$r->copyCauses('eng');
	//$a = $i->importFromCSV('/tmp/JAM_event.csv', DI_EVENT, true, 0);
	//$a = $i->importFromCSV('/tmp/JAM_cause.csv', DI_CAUSE, true, 0);
	//$a = $i->importFromCSV('/tmp/JAM_geolevel.csv', DI_GEOLEVEL, true, 0);
	//$a = $i->importFromCSV('/tmp/JAM_geography.csv', DI_GEOGRAPHY, true, 0);
	//$a = $i->importFromCSV('/tmp/JAM_disaster.csv', DI_DISASTER, true, 0);

	//createEEFields($us, '/tmp/JAM_eefield.csv','/tmp/JAM_eedata.csv');
	//importEEData($us,'/tmp/JAM_eedata.csv');
