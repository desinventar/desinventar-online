...
<script language="php">
	...
	//$r->copyCauses('eng');
	$i = new DIImport($us);
	//$a = $i->importFromCSV('/tmp/BLZ_event.csv', DI_EVENT, true, 0);
	//$a = $i->importFromCSV('/tmp/BLZ_cause.csv', DI_CAUSE, true, 0);
	//$a = $i->importFromCSV('/tmp/BLZ_geolevel.csv', DI_GEOLEVEL, true, 0);
	//$a = $i->importFromCSV('/tmp/BLZ_geography.csv', DI_GEOGRAPHY, true, 0);
	//$a = $i->importFromCSV('/tmp/BLZ_disaster.csv', DI_DISASTER, true, 0);

	//createEEFields($us, '/tmp/BLZ_eefield.csv','/tmp/BLZ_eedata.csv');
	//importEEData($us,'/tmp/BLZ_eedata.csv');
