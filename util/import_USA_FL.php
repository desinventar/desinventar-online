...
<script language="php">
	...
	//$r->copyEvents('eng');
	//$r->copyCauses('eng');
	//$a = $i->importFromCSV('/tmp/FL_event.csv', DI_EVENT, true, 0);
	//$a = $i->importFromCSV('/tmp/FL_cause.csv', DI_CAUSE, true, 0);
	//$a = $i->importFromCSV('/tmp/FL_geolevel.csv', DI_GEOLEVEL, true, 0);
	//$a = $i->importFromCSV('/tmp/FL_geography.csv', DI_GEOGRAPHY, true, 0);
	//$a = $i->importFromCSV('/tmp/FL_disaster.csv', DI_DISASTER, true, 0);

	//createEEFields($us, '/tmp/FL_eefield.csv','/tmp/FL_eedata.csv');
	//importEEData($us,'/tmp/FL_eedata.csv');
