...
<script language="php">
	...
	//$r->copyEvents('eng');
	//$r->copyCauses('eng');
	//$a = $i->importFromCSV('/tmp/TTO_event.csv', DI_EVENT, true, 0);
	//$a = $i->importFromCSV('/tmp/TTO_cause.csv', DI_CAUSE, true, 0);
	//$a = $i->importFromCSV('/tmp/TTO_geolevel.csv', DI_GEOLEVEL, true, 0);
	//$a = $i->importFromCSV('/tmp/TTO_geography.csv', DI_GEOGRAPHY, true, 0);
	//$a = $i->importFromCSV('/tmp/TTO_disaster.csv', DI_DISASTER, true, 0);

	//createEEFields($us, '/tmp/TTO_eefield.csv','/tmp/TTO_eedata.csv');
	//importEEData($us,'/tmp/TTO_eedata.csv');
