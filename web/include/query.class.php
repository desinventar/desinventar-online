...
class Query
{
    ...
    public function openSqliteDatabase($file_name)
    {
        if (!file_exists($file_name)) {
            showErrorMsg(debug_backtrace(), null, '');
            throw new Exception('Cannot find database file: ' . $file_name);
        }
        try {
    ...
    public function loadEvents($type, $status, $LangIsoCode, $RegionLangIsoCode, $withTranslate = true)
    {
        $data = array();
        if ($type == 'BASE') {
    ...
    public function loadCauses($type, $status, $LangIsoCode, $RegionLangIsoCode, $withTranslate = true)
    {
        $data = array();
        if ($type == 'BASE') {
    ...
    public function buildGeographyId($fid, $lev)
    {
    ...
    protected function getGeoNameById($geoid)
    {
        if ($geoid == "") {
            return null;
        }
    ...
    protected function buildGeoTree($child, $mylev, $maxlev, $selgeolist)
    {
        ...
        if ($maxlev >= $mylev) {
            foreach ($this->loadGeoChilds($child) as $gkey => $gitem) {
                ...
                if ($gitem[2]) {
                    $gtree[$gkey .'|'. $gitem[1] .'|'. $chked] = $this->buildGeoTree(
                        $gkey,
                        $mylev+1,
                        $maxlev,
                        $selgeolist
    ...
    public function loadGeography($level, $prmOnlyActive = true)
    {
        $data = array();
    ...
    public function loadGeoChilds($geoid, $prmOnlyActive = true)
    {
        $data = array();
        ...
        $res = $this->dreg->query($sql, PDO::FETCH_ASSOC);
        foreach ($res as $row) {
            $data[$row['GeographyId']] = array_merge(
                $row,
                [
                    $row['GeographyCode'],
                    $row['GeographyName'],
    ...
    public function getNextLev($geoid)
    {
    ...
    public function loadGeoLevels($prefix, $lev, $mapping)
    {
    ...
    public function loadGeoLevById($geolevid)
    {
    ...
    public function getEEFieldSeries()
    {
    ...
    public function getCountryByCode($idcnt)
    {
    ...
    public function checkExistsRegion($rid)
    {
    ...
    public function getRegionList($cnt, $status)
    {
        ...
        $sql = "
            SELECT RegionId, RegionLabel
            FROM Region
            WHERE IsCRegion=0 AND IsVRegion=0 AND $opt
    ...
    protected function querySQLAddTextField($prmQuery, $prmField, $prmValue, $prmOp)
    {
        $Value = trim($prmValue);
        $Query = $prmQuery;
        if ($Value != '') {
    ...
    protected function querySQLAddMemoField($prmQuery, $prmField, $prmValue, $prmOp)
    {
        $Value = trim($prmValue);
        $Query = $prmQuery;
        if ($Value != '') {
    ...
    public function genSQLWhereDesconsultar($dat)
    {
                        ...
                        if (isset($QueryParams['Operator'])) {
                            switch ($QueryParams['Operator']) {
                                ...
                                case '<=':
                                    if (is_numeric($QueryParams['Value1'])) {
                                        $QueryTmp =
                                            'E.' . $EEField . $QueryParams['Operator'] . $QueryParams['Value1'] .
                                            ' AND ' .
                                            'E.' . $EEField . '>=0';
                                    }
                                    break;
                                ...
                                case '-3':
                                    if (is_numeric($QueryParams['Value1']) && is_numeric($QueryParams['Value2'])) {
                                        $QueryTmp = '(' .
                                            'E.' . $EEField . '>=' . $QueryParams['Value1'] . ' AND ' .
                                            'E.' . $EEField . '<=' . $QueryParams['Value2'] . ')';
                                    }
                                    break;
                                default:
        ...
        $Query = $this->querySQLAddMemoField(
            $Query,
            'D.DisasterSiteNotes',
            $dat['D.DisasterSiteNotes'][1],
            $dat['QueryGeography']['OP']
        );
        ...
        $Query = $this->querySQLAddTextField(
            $Query,
            'D.EventDuration',
            $dat['D.EventDuration'],
            $dat['QueryEvent']['OP']
        ...
        $WhereQuery  = "WHERE ";
        $WhereQuery .= '(
            D.DisasterId=E.DisasterId AND
            D.EventId=V.EventId AND
            D.CauseId=C.CauseId AND
            D.GeographyId=G.GeographyId) ';
    ...
    public function printResults($dl, $exp, $mode)
    {
        $txt = '';
        // Get results
        if (!empty($dl)) {
                ...
                foreach (array_keys($i) as $idx) {
                    if (substr($idx, 0, 11) == "GeographyId") {
                        switch ($mode) {
                            ...
                            case "CODENAME":
                                $dl[$j][$idx] =
                                    $this->getObjectNameById($i[$idx], DI_GEOGRAPHY) .
                                    " | " .
                                    $this->getGeoNameById($i[$idx]);
                                break;
                            default:
                                $dl[$j][$idx] = "";
                                break;
    ...
    protected function hash2json($dlist)
    {
        $js = array();
    ...
    public function getQueryDetails($dic, $post)
    {
        $info = $lsf = array();
        $dinf = $this->getDBInfo();
        $info['TITLE'] = "";
    ...
    protected function existLang($LangIsoCode)
    {
        $answer = false;
        if ($LangIsoCode != "") {
    ...
    public function queryLabel($labgrp, $labname, $langID)
    {
        $data = '';
    ...
    public function querySecLabelFromGroup($labgrp, $langID)
    {
        $sql = "select g.LGName as lgn, g.LabelName as lbn, DictTranslation, ".
            "DictTechHelp, DictBasDesc, DictFullDesc from Dictionary d,".
            " LabelGroup g where (g.LGName like '". $labgrp ."%') and ".
    ...
    public function loadAllGroups($langID)
    {
        $sql = "select g.LGName as lgn, g.LabelName as lbn, DictTranslation, ".
            "DictTechHelp, DictBasDesc, DictFullDesc from Dictionary d,".
    ...
    public function loadLanguages($prmLangStatus = '')
    {
    ...
    protected function findDicLabelID($labgrp, $labname)
    {
    ...
    protected function existLabel($diclabelID)
    {
        $answer = false;
    ...
    public function updateDicLabel($labgrp, $labname, $translation, $techhelp, $basdesc, $fulldesc, $langID)
    {
