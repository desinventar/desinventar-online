  ...
  jQuery('div.Geography table.List tbody')
    ...
    */
    ...
    .on('click', 'tr', function() {
      ...
      jQuery('.GeographyActive', form).val(
        jQuery('.GeographyActive', this).text()
      )
      jQuery('.GeographyActiveCheckbox', form)
        .prop('checked', parseInt(jQuery('.GeographyActive', this).text()) > 0)
        .change()
      ...
      function(data) {
        if (parseInt(data.Status) > 0) {
          ...
          jQuery.each(data.GeolevelsList, function(key, value) {
            var clonedCell = jQuery('div.Geography table.ListHeader tr td:last')
              .clone()
              .show()
            jQuery(clonedCell).data('GeoLevelId', key)
            jQuery('span.title', clonedCell).text(
              parseInt(key + 1) + ' - ' + value.GeoLevelName
            )
            jQuery('select', clonedCell).data('GeoLevelId', key)
            jQuery('div.Geography table.ListHeader tr').append(clonedCell)
            jQuery(
              'div.Geography select.ListHeader:data("GeoLevelId=' +
                value.GeoLevelId +
                '")'
            ).disable()
          })
          jQuery('div.Geography input.GeoLevelCount').val(
            data.GeolevelsList.length
          )
...
function populate_geography_list(prmGeographyList) {
  ...
  var geolevel_count =
    parseInt(jQuery('div.Geography input.GeoLevelCount').val()) - 1
  ...
  var select = jQuery(
    'div.Geography select.ListHeader:data("GeoLevelId=' + prmGeoLevelId + '")'
  )
  select.empty()
  select.append(
    jQuery('<option>', { value: prmParentId }).text(
      jQuery('div.Geography span.All').text()
    )
  )
  ...
  jQuery.each(prmGeographyList, function(key, value) {
    if (prmGeoLevelId < geolevel_count) {
      select.append(
        jQuery('<option>', { value: key }).text(value.GeographyName)
      )
    }
    ...
    jQuery('.GeographyStatus', clonedRow).html(
      jQuery(
        'select.GeographyStatusText option[value="' +
          value.GeographyActive +
          '"]'
      ).text()
    )
